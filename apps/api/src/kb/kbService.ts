...
export async function indexFileToKb(input: {
  ...
  const existing = await listKbChunkDigestsForFile({
    clientId: input.clientId,
    sourceId: input.source.id,
    path: input.path,
  }).catch(() => []);
  const existingByRange = new Map<string, { hash: string; hasEmbedding: boolean }>();
  for (const e of existing) {
    existingByRange.set(`${e.startLine}:${e.endLine}`, { hash: e.contentHash, hasEmbedding: e.hasEmbedding });
  }
  ...
  for (const c of chunks) {
    ...
    const prev = existingByRange.get(key);
    const shouldEmbed = !prev || prev.hash !== hash || !prev.hasEmbedding;
    const embedded = shouldEmbed ? await embedText({ text: c.content }).catch(() => null) : null;
  ...
  // Remove any old chunks for this file that are no longer present after re-chunking.
  await deleteKbChunksNotInRangesForFile({
    clientId: input.clientId,
    sourceId: input.source.id,
    path: input.path,
    keep: chunks.map((c) => ({ startLine: c.startLine, endLine: c.endLine })),
  }).catch(() => undefined);
